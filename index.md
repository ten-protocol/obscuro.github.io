...
Any Verifier can quickly become an Aggregator by paying the stake and registering with the Management Contract.

Note that the TEN network has a permissionless design, where _anyone_ can run a node.

## Rollup Data Structure

This is the high-level rollup structure:

```
rollup = {
    header: {
        ParentHash,         // Hash(previous_rollup)
        L1Proof,            // Merkle proof of block inclusion 
        signature,          // Signature of the rollup hash signed by the enclave key 
    },
    txs: {
        EncryptedTxBlob,    // All the transactions encoded and encrypted together
        txs_hash,           // the hash of all the txs together
    }
}
```

A rollup is a data structure cryptographically signed by a TEE, which contains references to the L1 block that includes the previous rollup, all the encrypted transactions and some custom application data (like withdrawal requests which have to be in plaintext for the Bridge Contract to process).

While the rollup header is in plaintext and visible to everyone, the rollup body is encrypted, and it can only be decoded by a node with a valid TEE in possession of the shared secret.

All the L1 logic works exclusively with the rollup header.

The rollup also includes a hash of the L2 state after all the transactions are processed, which allows the Management Contract on L1 to efficiently check whether two rollups lead to the same state.

This diagram shows the linked structure of rollups:
![rollup structure](https://raw.githubusercontent.com/ten-protocol/ten-whitepaper/refs/heads/main/images/rollup-data-structure.png)
...
TEN uses hybrid encryption combining traditional cryptographic primitives with Trusted Execution Environment capabilities to provide confidentiality while maintaining transparency and auditability.

### Master Secret and Key Derivation

The TEN network operates using a shared master secret known only to attested TEEs. This master secret is:

1. **Generated** by the first TEE (genesis enclave) to join the network
2. **Encrypted** using each new TEE's attestation key before sharing
3. **Used** to derive transaction encryption keys and state encryption keys
4. **Rotated** periodically for forward secrecy

### Transaction Encryption

All user transactions are encrypted before being included in rollups using AES-256-GCM with keys derived from the master secret. This ensures that:

- Only valid TEEs can decrypt and execute transactions
- Transaction content remains hidden from node operators
- MEV extraction through transaction analysis is prevented

### State Encryption

The TEN state is maintained in encrypted form within TEE memory, with keys derived from the master secret. State persistence uses sealed storage where data is encrypted such that only the originating TEE can decrypt it.

### Attestation and Key Exchange

New nodes joining the network must:

1. **Generate** an attestation report proving their TEE integrity
2. **Submit** this report to the management contract for validation
3. **Receive** the encrypted master secret from existing network participants
4. **Prove** possession of the master secret to begin participating

## Account-Based State Model

TEN implements an account-based state model compatible with Ethereum's approach, enabling seamless migration of existing dApps while adding privacy features.

### Account Structure

Each TEN account maintains:

```
Account = {
    nonce: uint64,           // Transaction counter
    balance: uint256,        // ETH balance
    storageRoot: bytes32,    // Root hash of storage trie
...
All account state is encrypted within TEE memory using keys derived from the master secret. This includes:

- Account balances and nonces
- Contract storage slots
- Contract code (when deployed)

### View Functions and Privacy

TEN introduces programmable privacy for smart contract view functions:

- **Public views**: Accessible to anyone (like traditional blockchain)
- **Private views**: Only accessible to authorized parties
- **Conditional views**: Access based on caller's permissions or other criteria

## L1 Contracts

TEN's Layer 1 contracts manage the protocol's interaction with Ethereum mainnet, handling node registration, rollup validation, and asset bridges.

### Management Contract

The central L1 contract that coordinates the TEN network:

**Functions:**
- **Node Registration**: Validates TEE attestations and manages node stake
- **Attestation Constraints**: Maintains requirements for valid TEE software
- **Upgrade Management**: Handles protocol upgrades and TCB recovery
...
struct AttestationConstraints {
    bytes32[] validCodeHashes;  // Approved software hashes
    uint256 minSecurityLevel;   // Minimum security requirements
    address[] auditors;         // Approved auditor addresses
}
```

### Bridge Contract

Manages asset transfers between L1 and L2:

**Deposits:**
- Users deposit ERC-20 tokens to receive wrapped tokens on TEN
- Implements dependency mechanism linking L2 credits to L1 block finality
- Prevents double-spending through rollup invalidation on reorgs

**Withdrawals:**
- Processes withdrawal requests from finalized rollups
- Implements safety mechanisms against TEE compromise
- Enforces withdrawal delays and challenge periods

### Rollup Contract

Accepts and validates rollup submissions:

**Validation Rules:**
- Verifies rollup signatures from registered aggregators
- Ensures proper parent-child relationships in rollup chain
- Implements POBI consensus rules for canonical chain selection

**Data Management:**
- Stores rollup headers and metadata
- Maintains withdrawal request queues
- Tracks rollup finality status
